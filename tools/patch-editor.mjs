...
if (revert) {
  ...
  console.log("  ・メニューの「設定」→「フレーズ分割辞書」が増えます");
  console.log("  ・両方の辞書画面で単語を検索できるようになります（正規表現も可）");
  console.log("  ・単語の登録時にフレーズ分割辞書へ同時登録できるようになります");
  console.log("  ・書き出しの通知から保存先のフォルダを開けるようになります");
  console.log("  ・辞書を書き換えると、合成済みの音声と読みが作り直されます");
